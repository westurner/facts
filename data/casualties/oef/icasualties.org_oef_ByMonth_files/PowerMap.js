...
function updatePrvMap(val){	
			var myURL = "";
			var getCoalition = val;
			myURL = "Provincexml.aspx?Country=" + val;
			var mapObj = getMapFromId("map1Id");
			//Update it's URL
			mapObj.setDataURL(myURL);
			 $.get("ProvinceData.aspx?param=" + val + "&Item=Province", function(response) {
                    $("#mapdivLeft").empty();
                    $("#mapdivLeft").append(response);              
                });
			 $.get("ProvinceData.aspx?param=" + val + "&Item=Country", function(response) {
                    $("#dCountry").empty();
                    $("#dCountry").append(response);              
                });
            $.get("ProvinceData.aspx?param=" + val + "&Item=Detail", function(response) {
                    $("#dDetail").empty();
                    $("#dDetail").append(response);
                });   
            }
            function updateUSMap(val) {
                var myURL = "";

                var Type = document.getElementById('ddType').selectedIndex;
                myURL = "USCasualtiesByStateXML.aspx?Type=" + Type;
                var mapObj = getMapFromId("map1Id");
                //Update it's URL
                mapObj.setDataURL(myURL);
            }		